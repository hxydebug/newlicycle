...
//   std::cout<<this->count<<std::endl;
  ...
  if(leg==l_leg)  tau = calcu_Jaco(angles.head(3),leg).transpose() * rot_matrix.transpose() * F;
...
void stance_control::get_start(Leg leg,float roll,float dvarphi){
  if((current_state[leg] != pre_state[leg]) && (current_state[leg] == SWING_DOWN)){
	...
	if(USE_Optimizer==1){
		...
		float L = hL*sin(fabs(roll))+wide/2*cos(roll)+dL;
...
Position pos_control(Leg leg, double roll){
	...
	if(leg == l_leg){
		pos_H[0] = 0.20;
		pos_H[1] = dL+0.01;
		pos_H[2] = -legbase_H*cos(roll)+wide/2*sin(fabs(roll)); 
	}
	else{
		pos_H[0] = 0.20;
		pos_H[1] = -dL;
